...
const CreateCampaign = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 bg-white rounded-xl shadow-md mt-8 mb-10">
      <h2 className="text-2xl font-bold mb-6 text-center text-[#018AE0]">Create Donation Campaign</h2>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div>
          ...
          />
          {errors.petImage && <p className="text-red-500 text-sm">{errors.petImage.message}</p>}
        </div>

        {/* Maximum Donation */}
        <div>
          <label className="block mb-1 font-semibold">Maximum Donation Amount (৳)</label>
          <input
            type="number"
            step="0.01"
            {...register("maxDonation", { required: "Maximum amount is required" })}
            className="block w-full p-2 border border-gray-300 rounded"
          />
        ...
        {/* Last Date */}
        <div>
          <label className="block mb-1 font-semibold">Last Date of Donation</label>
          <input
            type="date"
